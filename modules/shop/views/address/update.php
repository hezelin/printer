...
border-radius: 4px; text-align: left; border: 1px solid #cccccc; font-size: 16px;}
    ...
    <form id="tbl_address_form" method="post" action="<?=\yii\helpers\Url::toRoute(['update','id'=>$id,'address_id'=>$model->id,'url'=>Yii::$app->request->get('url')])?>">
        <input type="hidden" id="tbl_address_city" name="TblShopAddress[city]" value="<?=$model->city?>" />
        <input class="tbl_address_row" type="text" id="tbl_address_name" name="TblShopAddress[name]" value="<?=$model->name?>" placeholder="姓名"/>
        <input class="tbl_address_row" type="text" id="tbl_address_phone" name="TblShopAddress[phone]" value="<?=$model->phone?>" placeholder="手机"/>
        ...
        <textarea class="tbl_address_area" id="tbl_address_area" name="TblShopAddress[address]" placeholder="街道地址"><?=$model->address?></textarea>
...
<script>
    ...
    $(function(){
        ...
        // 初始化 城市数据
        // 编辑
        var provinceVal = <?=substr($model->address_id,0,2)?>;
        var cityVal = <?=substr($model->address_id,0,4)?>;
        var regionVal = <?=$model->address_id?>;
        $('#province option[value="'+provinceVal+'"]').attr("selected","selected");
        $.getJSON('/shop/address/ajax?id='+provinceVal,function(result){
            if( result.status == 1){
                $('#city').html( result.data);
                $('#city option[value="'+cityVal+'"]').attr("selected","selected");
            }
        });
        $.getJSON('/shop/address/ajax?id='+cityVal,function(result){
            if( result.status == 1){
                $('#region').html( result.data);
                $('#region option[value="'+regionVal+'"]').attr("selected","selected");
            }
        });
    })
    <?php $this->endBlock();?>
